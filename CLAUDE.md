...
| `routers/arena.py` | `/api/arena` | Pilot Arena 模型加载、单帧推理、批量预测与预览图 |

`drive.py` 的 WebSocket 通道通过 `role=car|client` 区分车端和浏览器连接，车端推送图像帧和状态，浏览器发送控制指令，服务端做双向代理和状态缓存。

`arena.py` 复用当前加载的 Tub 数据，并在服务端维护已加载 Pilot 与推理缓存；前端通过 `PilotArenaPage.tsx` 并排比较多个模型，图像播放与推理解耦。

**前端** (`web_ui/frontend/`)：React 18 + TypeScript + TailwindCSS + Zustand
- 开发服务器：Vite，端口 5188，`/api` 代理到 `localhost:8000`
- 路由（HashRouter）：`/` Tub 管理、`/trainer` 训练、`/drive` 驾驶、`/calibrate` 校准、`/arena` Pilot Arena
...
python -m pytest tests/test_arena.py tests/test_config.py -q  # 后端路由测试
python -m pytest tests/test_arena.py::test_predict_returns_user_and_pilot_values -q  # 单用例
...
npm run check                             # TypeScript noEmit 检查
```
